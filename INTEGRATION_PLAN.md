...
1. **Primitives only.** Port `oauth1.py` and the signing/HTTP logic in
...
2. **Credentials and storage.** Add the per-tenant Secret Manager config
...
3. **The three-leg flow as routes** (`start` takes `tenant_uuid` in its body,
...
4. **Read path.** Wire `get-specialist` / `find-specialist` into whichever
   service owns the domain mapping decided above, behind a feature flag.
5. **Write path.** Add `add-specialist` once the read path is validated
   against the pilot tenant.
6. **Expiry detection.** Add the Cloud Scheduler job + structured alert
...
7. **Per-tenant onboarding.** Roll out to further tenants one at a time —
...
8. **Retire or keep the standalone tool.** Once the pilot tenant is validated
